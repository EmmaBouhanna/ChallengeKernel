...
def main():
    ...
    # Load training label
    train_labels = load_data('./data/training_labels.pkl',
                             labels=True, neg_class=NEGATIVE_CLASS)

    # Get folds for crossvalidation
    folds = get_folds(N_FOLDS, train_labels, NEGATIVE_CLASS,
    ...
    idx_best_train = np.setdiff1d(
        np.arange(len(train_labels)), folds[best_fold])
    ...
    dataframe.index += 1
    dataframe.to_csv('kaggle_submissions/test_pred.csv',
                     index_label='Id')
